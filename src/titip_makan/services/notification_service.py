...
def format_session_announcement(session: SessionOut, public_url: Optional[str] = None) -> str:
    ...
    return (
        f"📢 *PENGUMUMAN TITIP MAKAN MTN CORE* 🍜\n\n"
...
class NotificationService:
    ...
    async def broadcast_session_opened(
        ...
        message = format_session_announcement(session, self.public_url)
        try:
            res = await self.send_text(target_chat_id, message)
            if res.get("status") == "skipped":
                return res
            logger.info(f"WA announcement sent to {target_chat_id}: {res}")
